...
int thead_mfg_enable(struct device *dev, struct regmap *vosys_regmap)
{
	struct clk *cclk, *aclk;
	int ret;
	int val;
	...
	if (ret) {
		dev_err(dev, "failed to enable clock cclk\n");
		return -EINVAL;
	}
	...
	regmap_read(vosys_regmap, 0x0, &val);
	if (val) {
		regmap_update_bits(vosys_regmap, 0x0, 3, 0);
		regmap_read(vosys_regmap, 0x0, &val);
		if (val) {
			pr_info("[GPU_RST]"
				"val is %x\r\n",
	...
	/* rst gpu clkgen */
	regmap_update_bits(vosys_regmap, 0x0, 2, 2);
	regmap_read(vosys_regmap, 0x0, &val);
	if (!(val & 0x2)) {
		pr_info("[GPU_CLK_RST]"
			"val is %x\r\n",
	...
	udelay(1);
	/* rst gpu */
	regmap_update_bits(vosys_regmap, 0x0, 1, 1);
	regmap_read(vosys_regmap, 0x0, &val);
	if (!(val & 0x1)) {
		pr_info("[GPU_RST]"
			"val is %x\r\n",
...
EXPORT_SYMBOL_GPL(thead_mfg_enable);

void thead_mfg_disable(struct regmap *vosys_regmap)
{
	int val;
	regmap_update_bits(vosys_regmap, 0x0, 3, 0);
	regmap_read(vosys_regmap, 0x0, &val);
...
static int light_vosys_clk_probe(struct platform_device *pdev)
{
	...
	//vosys_regmap = syscon_regmap_lookup_by_phandle(dev->of_node, "vosys-regmap");
