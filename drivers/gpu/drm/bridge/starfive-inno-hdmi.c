...
#include <linux/mfd/core.h> /* For MFD parent access */
#include <linux/of.h>
#include <linux/of_device.h>
#include <linux/platform_device.h>
#include <linux/phy/phy.h> /* For PHY handle */
...
/* Input clocks for the controller */
				      ...
				      struct drm_display_mode *mode)
{
	...
	struct inno_hdmi *hdmi = ctrl->inno;
	int ret;

	printk("MICHAL inno hdmi starfive enable\n");
	...
	if (ret)
		dev_err(dev, "Failed to set pclk rate %lu: %d\n",
			mode->clock * 1000, ret);
	...
	printk("MICHAL inno hdmi starfive enable 2\n");

	ctrl->vic = drm_match_cea_mode(mode);
}

/* We must add a .disable hook to power down the PHY */
...
static int starfive_inno_hdmi_controller_probe(struct platform_device *pdev)
{
	...
	/* Enable register access clocks (sys, mclk, bclk) */
	 ...
	 * This assumes the generic inno_hdmi.c is patched to
	 * accept a regmap from the parent.
	 */
	...
	/*
	 * Force the generic library to use the MFD regmap.
	 * This is the magic that makes the MFD split work.
	 */
	ctrl->inno->regmap = mfd_regmap;
	ctrl->inno->regs = NULL; /* Prevent use of ioremap */

	printk("MICHAL successfully registered inno hdmi CTL\n");
...
static void starfive_inno_hdmi_controller_remove(struct platform_device *pdev)
{
	...
	reset_control_assert(ctrl->tx_rst);
	clk_bulk_disable_unprepare(CLK_CTRL_NUM, ctrl->clks);
}
...
static const struct inno_hdmi_plat_ops stf_inno_hdmi_plat_ops = {
	...
	//.disable = inno_hdmi_starfive_disable, /* Added disable hook */
};
