 ...
 */
static const u16 RGB2RGB[RGB_TO_RGB_TABLE_SIZE] = { 10279, 5395,  709,
						    1132,  15065, 187,
						    269,   1442,  14674 };
...
static void vs_dc_load_plane_default_filter(struct dc_hw *hw,
					    const struct dc_hw_plane_reg *reg,
					    u32 offset)
{
	u8 i;

	vs_dc_write(hw, reg->scale_config + offset, 0x33);
	vs_dc_write(hw, reg->init_offset + offset, 0x80008000);
	vs_dc_write(hw, reg->h_filter_coef_index + offset, 0x00);
	for (i = 0; i < H_COEF_SIZE; i++)
		vs_dc_write(hw, reg->h_filter_coef_data + offset, horkernel[i]);

	vs_dc_write(hw, reg->v_filter_coef_index + offset, 0x00);
	for (i = 0; i < V_COEF_SIZE; i++)
		vs_dc_write(hw, reg->v_filter_coef_data + offset, verkernel[i]);
}
...
/**
 *vs_dc_load_plane_rgb_csc- Load RGB to RGB conversion coefficient
 *
 ...
 */
static void vs_dc_load_plane_rgb_csc(struct dc_hw *hw,
				     const struct dc_hw_plane_reg *reg,
...
/**
 * vs_dc_load_plane_yuv_to_rgb_csc - Load YUV to RGB conversion coefficients into hardware registers
 ...
 */
static void vs_dc_load_plane_yuv_to_rgb_csc(struct dc_hw *hw,
					    const struct dc_hw_plane_reg *reg,
					    u32 offset, const s32 *table)
{
	...
	u32 regval[ARRAY_SIZE(coef_reg)] = {
		(0xFFFF & table[0]) |
			(table[1]
			 << 16), //Lower 16 of tab[0] and upper 16 of tab[1]
		(0xFFFF & table[2]) |
			(table[3]
			 << 16), //Lower 16 of tab[2] and upper 16 of tab[3]
		(0xFFFF & table[4]) |
			(table[5]
			 << 16), //Lower 16 of tab[4] and upper 16 of tab[5]
		(0xFFFF & table[6]) |
			(table[7]
			 << 16), //Lower 16 of tab[6] and upper 16 of tab[7]
		...
		table[12] |
			(table[13]
			 << 16), //Lower 16 of tab[12] and upper 16 of tab[13]
		table[14] |
			(table[15]
			 << 16), //Lower 16 of tab[14] and upper 16 of tab[15]
	};
 ...
 */
static void vs_dc_load_crtc_rgb_to_yuv_csc(struct dc_hw *hw, u32 offset,
					   s16 *table)
{
	u32 coef_reg[] = {
	...
	u32 regval[ARRAY_SIZE(coef_reg)] = {
		table[0] |
			(table[1]
			 << 16), // Lower 16 of table[0] and upper 16 of table[1]
		table[2] |
			(table[3]
			 << 16), // Lower 16 of table[2] and upper 16 of table[3]
		table[4] |
			(table[5]
			 << 16), // Lower 16 of table[4] and upper 16 of table[5]
		table[6] |
			(table[7]
			 << 16), // Lower 16 of table[6] and upper 16 of table[7]
...
{
	if (hw_id == PRIMARY_PLANE_0 || hw_id == PRIMARY_PLANE_1)
		return &dc_plane_reg[0];
	else
		return &dc_plane_reg[1];
}

int vs_dc_hw_init(struct vs_drm_device *priv)
{
	u8 i, panel_num, layer_num;
	...
	for (i = 0; i < layer_num; i++) {
		...
		vs_dc_load_plane_default_filter(hw, plane->reg, plane->offset);
		vs_dc_load_plane_rgb_csc(hw, plane->reg, plane->offset,
					 RGB2RGB);
	}
	...
	for (i = 0; i < panel_num; i++) {
		...
		vs_dc_write(hw, DC_DISPLAY_PANEL_CONFIG + offset,
			    PANEL_DE_EN | PANEL_DATA_EN | PANEL_CLOCK_EN);
...
{
	struct vs_drm_device *priv = to_vs_drm_private(plane->base.dev);
	struct dc_hw *hw = &priv->hw;

	u32 offset = plane->offset;
	u32 hw_id = plane->hw_id;

	if (hw_id == PRIMARY_PLANE_0 || hw_id == PRIMARY_PLANE_1)
		vs_dc_write_mask(hw, DC_FRAMEBUFFER_CONFIG_EX + offset,
				 PRIMARY_EN(false), PRIMARY_EN_MASK);
	else
		vs_dc_write_mask(hw, DC_OVERLAY_CONFIG + offset,
				 OVERLAY_FB_EN(false), OVERLAY_FB_EN_MASK);
}

static int vs_dc_get_cursor_size(u32 crtc_w)
{
	switch (crtc_w) {
	case 32:
...
void vs_dc_hw_update_cursor(struct vs_drm_device *priv, u8 id,
			    ...
			    u32 crtc_y, s32 hotspot_x, s32 hotspot_y)
{
	...
	size = vs_dc_get_cursor_size(crtc_w);

	vs_dc_write(hw, DC_CURSOR_ADDRESS + offset, dma_addr);
	vs_dc_write(hw, DC_CURSOR_LOCATION + offset,
		    X_LCOTION(crtc_x) | Y_LCOTION(crtc_y));
	vs_dc_write_mask(hw, DC_CURSOR_CONFIG + offset,
			 CURSOR_HOT_X(hotspot_x) | CURSOR_HOT_y(hotspot_y) |
				 CURSOR_SIZE(size) | CURSOR_VALID(1) |
				 CURSOR_TRIG_FETCH(1) |
				 CURSOR_FORMAT(CURSOR_FORMAT_A8R8G8B8),
			 CURSOR_HOT_X_MASK | CURSOR_HOT_y_MASK |
				 CURSOR_SIZE_MASK | CURSOR_VALID_MASK |
				 CURSOR_TRIG_FETCH_MASK | CURSOR_FORMAT_MASK);
}

void vs_dc_hw_disable_cursor(struct vs_drm_device *priv, u8 id)
{
	struct dc_hw *hw = &priv->hw;
	u32 offset = 0;

	offset = id ? DC_CURSOR_OFFSET : 0;
	vs_dc_clear_bit(hw, DC_CURSOR_CONFIG + offset, CURSOR_FORMAT_MASK);
	vs_dc_set_bit(hw, DC_CURSOR_CONFIG + offset, CURSOR_VALID(1));
}

void vs_dc_hw_update_gamma(struct vs_drm_device *priv, u8 id, u16 index, u16 r,
			   u16 g, u16 b)
{
	struct dc_hw *hw = &priv->hw;
...
void vs_dc_hw_enable_gamma(struct vs_drm_device *priv, u8 id, bool enable)
{
	...
	if (enable) {
		...
		for (int i = 0; i < GAMMA_EX_SIZE; i++) {
			...
			vs_dc_write(hw, DC_DISPLAY_GAMMA_EX_DATA + (id << 2),
				    value);
			vs_dc_write(hw,
				    DC_DISPLAY_GAMMA_EX_ONE_DATA + (id << 2),
				    hw->gamma[id].gamma[i][0]);
		}
		vs_dc_set_bit(hw, DC_DISPLAY_PANEL_CONFIG + (id << 2),
			      PANEL_GAMMA_EN);
	} else {
		vs_dc_clear_bit(hw, DC_DISPLAY_PANEL_CONFIG + (id << 2),
				PANEL_GAMMA_EN);
	}
}

void vs_dc_hw_enable(struct vs_drm_device *priv, int id,
		     ...
		     u32 output_fmt)
{
	...
	if (encoder_type != DRM_MODE_ENCODER_DSI) {
		...
		if (is_yuv)
			vs_dc_set_bit(hw, DC_DISPLAY_PANEL_CONFIG + offset,
				      PANEL_RGB2YUV_EN);
		else
			vs_dc_clear_bit(hw, DC_DISPLAY_PANEL_CONFIG + offset,
					PANEL_RGB2YUV_EN);
		vs_dc_write(hw, DC_DISPLAY_DP_CONFIG + offset,
			    dp_cfg | DP_SELECT);
	}
	...
	if (id == 0)
		vs_dc_clear_bit(hw, DC_DISPLAY_PANEL_START,
				PANEL0_EN | TWO_PANEL_EN);
	else
		vs_dc_clear_bit(hw, DC_DISPLAY_PANEL_START,
				PANEL1_EN | TWO_PANEL_EN);

	vs_dc_write(hw, DC_DISPLAY_H + offset,
		    H_ACTIVE_LEN(mode->hdisplay) | H_TOTAL_LEN(mode->htotal));

	vs_dc_write(hw, DC_DISPLAY_H_SYNC + offset,
		    H_SYNC_START_LEN(mode->hsync_start) |
			    H_SYNC_END_LEN(mode->hsync_end) |
			    H_POLARITY_LEN(mode->flags & DRM_MODE_FLAG_PHSYNC ?
						   0 :
						   1) |
			    H_PLUS_LEN(1));

	vs_dc_write(hw, DC_DISPLAY_V + offset,
		    V_ACTIVE_LEN(mode->vdisplay) | V_TOTAL_LEN(mode->vtotal));

	vs_dc_write(hw, DC_DISPLAY_V_SYNC + offset,
		    V_SYNC_START_LEN(mode->vsync_start) |
			    V_SYNC_END_LEN(mode->vsync_end) |
			    V_POLARITY_LEN(mode->flags & DRM_MODE_FLAG_PVSYNC ?
						   0 :
						   1) |
			    V_PLUS_LEN(1));
...
void vs_dc_hw_enable_shadow_register(struct vs_drm_device *priv, bool enable)
{
	...
	for (i = 0; i < layer_num; i++) {
		struct vs_plane *plane = priv->planes[i];
		hw_id = plane->hw_id;
		offset = plane->offset;

		if (enable) {
			if (hw_id == PRIMARY_PLANE_0 ||
			    hw_id == PRIMARY_PLANE_1)
				vs_dc_set_bit(hw,
					      DC_FRAMEBUFFER_CONFIG_EX + offset,
					      PRIMARY_SHADOW_EN);
			else
				vs_dc_set_bit(hw, DC_OVERLAY_CONFIG + offset,
					      OVERLAY_SHADOW_EN);
		} else {
			if (hw_id == PRIMARY_PLANE_0 ||
			    hw_id == PRIMARY_PLANE_1)
				vs_dc_clear_bit(
					hw, DC_FRAMEBUFFER_CONFIG_EX + offset,
					PRIMARY_SHADOW_EN);
			else
				vs_dc_clear_bit(hw, DC_OVERLAY_CONFIG + offset,
						OVERLAY_SHADOW_EN);
		}
	}
	...
	for (i = 0; i < panel_num; i++) {
		offset = i << 2;
		if (enable)
			vs_dc_clear_bit(hw, DC_DISPLAY_PANEL_CONFIG_EX + offset,
					PANEL_SHADOW_INVALID);
		else
			vs_dc_set_bit(hw, DC_DISPLAY_PANEL_CONFIG_EX + offset,
				      PANEL_SHADOW_INVALID);
	}
}
...
static inline u8 vs_dc_to_vs_yuv_color_space(u32 color_space)
{
	switch (color_space) {
...
static inline u8 vs_dc_to_vs_rotation(unsigned int rotation)
{
					     ...
					     bool is_yuv)
{
	...
	if (is_yuv) {
		if (hw_id == PRIMARY_PLANE_0 || hw_id == PRIMARY_PLANE_1) {
			vs_dc_clear_bit(hw, DC_FRAMEBUFFER_CONFIG_EX + offset,
					PRIMARY_RGB2RGB_EN);
			vs_dc_set_bit(hw, DC_FRAMEBUFFER_CONFIG_EX + offset,
				      PRIMARY_YUVCLAMP_EN);
		} else {
			vs_dc_clear_bit(hw, DC_OVERLAY_CONFIG + offset,
					OVERLAY_RGB2RGB_EN);
		...
		switch (vs_dc_to_vs_yuv_color_space(encoding)) {
		case COLOR_SPACE_601:
			vs_dc_load_plane_yuv_to_rgb_csc(hw, plane->reg, offset,
							YUV601_2RGB);
			break;
				   ...
				   struct drm_rect *src)
{
	...
	vs_dc_write(hw, plane->reg->y_address + offset, dma_addr[0]);
	vs_dc_write(hw, plane->reg->u_address + offset,
		    format == DRM_FORMAT_YVU420 ? dma_addr[2] : dma_addr[1]);
	vs_dc_write(hw, plane->reg->v_address + offset,
		    format == DRM_FORMAT_YVU420 ? dma_addr[1] : dma_addr[2]);
	vs_dc_write(hw, plane->reg->y_stride + offset, drm_fb->pitches[0]);
	vs_dc_write(hw, plane->reg->u_stride + offset,
		    format == DRM_FORMAT_YVU420 ? drm_fb->pitches[2] :
						  drm_fb->pitches[1]);
	vs_dc_write(hw, plane->reg->v_stride + offset,
		    format == DRM_FORMAT_YVU420 ? drm_fb->pitches[1] :
						  drm_fb->pitches[2]);
	vs_dc_write(hw, plane->reg->size + offset,
		    FB_SIZE(drm_rect_width(src) >> 16,
				  ...
				  unsigned int zpos, u8 display_id)
{
	...
	if (plane->hw_id == PRIMARY_PLANE_0 ||
	    plane->hw_id == PRIMARY_PLANE_1) {
		vs_dc_write_mask(
			hw, DC_FRAMEBUFFER_CONFIG + offset,
			PRIMARY_FORMAT(dc_update_vs_format(format)) |
				...
				PRIMARY_YUV_COLOR(
					vs_dc_to_vs_yuv_color_space(encoding)) |
				PRIMARY_ROTATION(
					vs_dc_to_vs_rotation(rotation)),
			PRIMARY_FORMAT_MASK | PRIMARY_UV_SWIZ_MASK |
				PRIMARY_SWIZ_MASK | PRIMARY_TILE_MASK |
				PRIMARY_YUV_COLOR_MASK | PRIMARY_ROTATION_MASK |
				PRIMARY_CLEAR_EN_MASK);
		vs_dc_write_mask(
			hw, DC_FRAMEBUFFER_CONFIG_EX + offset,
			PRIMARY_DECODER_EN(false) | PRIMARY_EN(visible) |
				PRIMARY_ZPOS(zpos) |
				PRIMARY_CHANNEL(display_id),
			PRIMARY_DECODER_EN_EN_MASK | PRIMARY_EN_MASK |
				PRIMARY_ZPOS_MASK | PRIMARY_CHANNEL_MASK);
	} else {
		vs_dc_write_mask(
			hw, DC_OVERLAY_CONFIG + offset,
			OVERLAY_FB_EN(visible) |
				...
				OVERLAY_ROTATION(
					vs_dc_to_vs_rotation(rotation)),
			OVERLAY_DEC_EN_MASK | OVERLAY_CLEAR_EN_MASK |
				OVERLAY_FB_EN_MASK | OVERLAY_FORMAT_MASK |
				OVERLAY_UV_SWIZ_MASK | OVERLAY_SWIZ_MASK |
				OVERLAY_TILE_MASK | OVERLAY_YUV_COLOR_MASK |
				OVERLAY_ROTATION_MASK);

		vs_dc_write_mask(
			hw, DC_OVERLAY_CONFIG_EX + offset,
				 ...
				 unsigned int rotation)
{
	...
	if (enable_scale) {
		vs_dc_write(hw, plane->reg->scale_factor_x + offset,
			    scale_factor_x);
		vs_dc_write(hw, plane->reg->scale_factor_y + offset,
			    scale_factor_y);
		if (plane->hw_id == PRIMARY_PLANE_0 ||
		    plane->hw_id == PRIMARY_PLANE_1)
			vs_dc_set_bit(hw, DC_FRAMEBUFFER_CONFIG + offset,
				      PRIMARY_SCALE_EN);
		else
			vs_dc_set_bit(hw, DC_OVERLAY_SCALE_CONFIG + offset,
				      OVERLAY_SCALE_EN);
	} else {
		if (plane->hw_id == PRIMARY_PLANE_0 ||
		    plane->hw_id == PRIMARY_PLANE_1)
			vs_dc_clear_bit(hw, DC_FRAMEBUFFER_CONFIG + offset,
					PRIMARY_SCALE_EN);
		else
			vs_dc_clear_bit(hw, DC_OVERLAY_SCALE_CONFIG + offset,
					OVERLAY_SCALE_EN);
	}

	vs_dc_write(hw, plane->reg->top_left + offset,
		    X_POS(dst->x1) | Y_POS(dst->y1));
	vs_dc_write(hw, plane->reg->bottom_right + offset,
		    X_POS(dst->x2) | Y_POS(dst->y2));
}
				 ...
				 u16 pixel_blend_mode, u8 display_id)
{
	...
	vs_dc_write(hw, plane->reg->src_global_color + offset,
		    PRIMARY_ALPHA_LEN(alpha >> 8));
	vs_dc_write(hw, plane->reg->dst_global_color + offset,
		    PRIMARY_ALPHA_LEN(alpha >> 8));
	switch (pixel_blend_mode) {
	case DRM_MODE_BLEND_PREMULTI:
		vs_dc_write(hw, plane->reg->blend_config + offset,
			    BLEND_PREMULTI);
		break;
