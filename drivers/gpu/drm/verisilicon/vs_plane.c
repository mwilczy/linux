...
static int vs_common_plane_atomic_check(struct drm_plane *plane, struct drm_atomic_state *state)
{
	...
	plane_info = (struct vs_plane_info *)&priv->hw.info->info[to_vs_plane(plane)->id];
...
static void vs_plane_atomic_update(struct drm_plane *plane, struct drm_atomic_state *state)
{
	...
	unsigned char i, num_planes, display_id;
	u32 format;
	bool is_yuv;
	...
	vs_dc_plane_hw_update_format_colorspace(vs_plane, format, new_state->color_encoding, is_yuv);
	if (new_state->visible)
		vs_dc_plane_hw_update_address(vs_plane, format, plane_state->dma_addr,
					   new_state->fb, &new_state->src);
	vs_dc_plane_hw_update_format(vs_plane, format, new_state->color_encoding, new_state->rotation,
				  new_state->visible, new_state->zpos, display_id);
	vs_dc_plane_hw_update_scale(vs_plane, &new_state->src, &new_state->dst,
				 display_id, new_state->rotation);
	vs_dc_plane_hw_update_blend(vs_plane, new_state->alpha, new_state->pixel_blend_mode,
				 display_id);
}
...
static void vs_cursor_plane_atomic_update(struct drm_plane *plane, struct drm_atomic_state *state)
{
	...
	vs_dc_hw_update_cursor(priv, display_id, dma_addr, crtc_w, crtc_x,
...
static void vs_plane_atomic_disable(struct drm_plane *plane, struct drm_atomic_state *state)
{
	...
	vs_dc_hw_disable_plane(vs_plane);
}
...
static void vs_cursor_plane_atomic_disable(struct drm_plane *plane, struct drm_atomic_state *state)
{
	...
	vs_dc_hw_disable_cursor(priv, display_id);
}
